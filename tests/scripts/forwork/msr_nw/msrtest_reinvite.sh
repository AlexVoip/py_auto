...
sudo rm $TEMP_PATH/results_msrreinvite.txt
...
REZULT ()
{
  if test $? -ne 0
      then
          ...
          echo Test $COUNT failed >> $TEMP_PATH/results_msrreinvite.txt
      else
          SUCC_COUNT=$(($SUCC_COUNT+1))
          echo Test $COUNT passed >> $TEMP_PATH/results_msrreinvite.txt
  fi
}
...
COUNT=1
sudo $SIPP_PATH/sipp $SSWIP -sf $SRC_PATH/1-uac_ssw.xml -inf $TEMP_PATH/$USER_A-$USER_B.csv -mi $SIPPIP -s $USER_A -ap $USER_A -rtp_echo -l 1 -r 1 -m 1 -nd -i $SIPPIP -p $PORT_A -recv_timeout 200s -timeout_error
REZULT
