...
sudo rm $TEMP_PATH/results_ss_cfnr2.txt
...
REZULT ()
{
  if test $? -ne 0
      then 
          ...
          echo Test $COUNT failed >> $TEMP_PATH/results_ss_cfnr2.txt
      else 
          SUCC_COUNT=$(($SUCC_COUNT+1))
          echo Test $COUNT passed >> $TEMP_PATH/results_ss_cfnr2.txt          
  fi
}
...
sudo $SIPP_PATH/sipp $1 -sf $SRC_PATH/2-uac_ssw.xml -inf $TEMP_PATH/${USER_MASS[7]}-${USER_MASS[9]}.csv -mi $2 -s ${USER_MASS[7]} -ap ${USER_MASS[7]} -rtp_echo -l 1 -r 1 -m 1 -nd -i $2 -p ${PORT_MASS[7]} -recv_timeout 200s -timeout_error
REZULT 
sleep 2
