...
# Тест услуги "Переадресация по недоступности" - CFU. Версия ПО 3.3.
# 
...
sudo rm $TEMP_PATH/results_ss_cfoos.txt
...
REZULT ()
{
  if test $? -ne 0
      then 
          ...
          echo Test $COUNT failed >> $TEMP_PATH/results_ss_cfoos.txt
      else 
          SUCC_COUNT=$(($SUCC_COUNT+1))
          echo Test $COUNT passed >> $TEMP_PATH/results_ss_cfoos.txt          
  fi
}

# Функция исключения нескольких элементов
IGNORE ()
{
if test $COUNT -eq 3
   then
...
echo "SEQUENTIAL;
${USER_MASS[$COUNT]};${USER_DOM_MASS[$COUNT]};*$CFOOS_KOD*${USER_MASS[$(($COUNT+1))]}#;*#$CFOOS_KOD#;#$CFOOS_KOD#;*$CFOOS_KOD#;" > $TEMP_PATH/${USER_MASS[$COUNT]}.csv
done
...
echo "SEQUENTIAL;
${USER_MASS[10]};${USER_DOM_MASS[10]};*$CFOOS_KOD*${USER_MASS[0]}#;" > $TEMP_PATH/${USER_MASS[10]}-max.csv

echo "SEQUENTIAL;
${USER_MASS[0]};${USER_DOM_MASS[0]};*$CFOOS_KOD*$USER_NR#;" > $TEMP_PATH/${USER_MASS[0]}-$USER_NR.csv
echo "SEQUENTIAL;
${USER_MASS[1]};${USER_DOM_MASS[1]};*$CFOOS_KOD*$USER_UA#;" > $TEMP_PATH/${USER_MASS[1]}-$USER_UA.csv 
echo "SEQUENTIAL;
${USER_MASS[2]};${USER_DOM_MASS[2]};*$CFOOS_KOD*$USER_NN#;" > $TEMP_PATH/${USER_MASS[2]}-$USER_NN.csv 
...
# Max redirect=5
sudo $SIPP_PATH/sipp $1 -sf $SRC_PATH/reg_auth.xml -inf $TEMP_PATH/${USER_MASS[10]}.csv -mi $2 -s ${USER_MASS[10]} -ap ${USER_MASS[10]} -rtp_echo -l 1 -r 1 -m 1 -nd -i $2 -p ${PORT_MASS[10]} -recv_timeout 200s -timeout_error
REZULT 
...
REZULT
sudo $SIPP_PATH/sipp $1 -sf $SRC_PATH/reg_auth0.xml -inf $TEMP_PATH/${USER_MASS[10]}.csv -mi $2 -s ${USER_MASS[10]} -ap ${USER_MASS[10]} -rtp_echo -l 1 -r 1 -m 1 -nd -i $2 -p ${PORT_MASS[10]} -recv_timeout 200s -timeout_error                 
REZULT 
