...
# Тест услуги "Переадресация безусловная" - CFU. Версия ПО 3.3.
# 
...
sudo rm $TEMP_PATH/results_ss_cfu2.txt
...
REZULT ()
{
  if test $? -ne 0
      then 
          ...
          echo Test $COUNT failed >> $TEMP_PATH/results_ss_cfu2.txt
      else 
          SUCC_COUNT=$(($SUCC_COUNT+1))
          echo Test $COUNT passed >> $TEMP_PATH/results_ss_cfu2.txt          
  fi
}
