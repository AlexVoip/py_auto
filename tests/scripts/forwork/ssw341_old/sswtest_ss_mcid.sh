...
REZULT ()
{
  if test $? -ne 0
      then 
          ...
          echo Test $COUNT failed >> $TEMP_PATH/results_ss_mcid.txt
      else 
          SUCC_COUNT=$(($SUCC_COUNT+1))
          echo Test $COUNT passed >> $TEMP_PATH/results_ss_mcid.txt          
  fi
}
...
sudo $SIPP_PATH/sipp $4 -sf $SRC_PATH/1-uac_ssw.xml -inf $TEMP_PATH/$USER_B.csv -mi $5 -s $USER_B -ap $USER_B -l 1 -r 1 -m 1 -nd -i $5 -p $PORT_B -recv_timeout 200s -timeout_error
REZULT
sleep 5

# MCID dial
COUNT=2
sudo $SIPP_PATH/sipp $4 -sf $SRC_PATH/uac_field2.xml -inf $TEMP_PATH/$USER_A-mcid.csv -mi $5 -s $USER_A -ap $USER_A -rtp_echo -l 1 -r 1 -m 1 -nd -i $5 -p $PORT_A -recv_timeout 200s -timeout_error            
REZULT
sleep 2
...
$SRC_PATH/sswconf_mcid2.sh $1 $2 $3 $4 $USER_A
